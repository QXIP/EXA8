...
docker run -ti --privileged --rm --name builder -v /usr/bin/qemu-aarch64-static:/usr/bin/qemu-aarch64-static -v $(pwd)/:/tmp/build -v $(pwd)/:/scripts --entrypoint=/scripts/release.sh arm64v8/ubuntu:xenial
